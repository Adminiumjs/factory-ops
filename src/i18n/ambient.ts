 ...
 *
 * `lib/format.ts` is a pure module: the zustand store, `data/demo.ts` and both
 * engines call its formatters from outside React, where no hook can reach the
 * provider. Rather than duplicate the runtime — a second lookup table and a
 * second set of `Intl` rules — `<App>` pushes the provider's own `locale` / `t`
 * / `money` / `number` in here on every render, so those callers forward to
 * exactly the functions the tree is using.
 *
...
const fallbackMoney: MoneyFn = (value, currency = "GBP") =>
...
/**
 * Lookup for keys assembled at runtime from data (`'data.item.' + sku`), where
 * the compiler cannot check the key. Returns `fallback` — the raw English from
 * the seed — when the bundle has nothing for it, so unknown seed data still
 * renders instead of leaking a dotted key onto the screen.
 */
